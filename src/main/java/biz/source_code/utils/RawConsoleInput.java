 ...
 *
 * <p>This class is used for console mode programs. It supports non-blocking reads of single key strokes without echo.
 */
public class RawConsoleInput {

    private static final boolean isWindows = System.getProperty("os.name").startsWith("Windows");
    private static final boolean isMacOSX = System.getProperty("os.name").startsWith("Mac OS X");
...
// The Windows version uses _kbhit() and _getwch() from msvcrt.dll.
     ...
     * @return -2 if <code>wait</code> is <code>false</code> and no character is available. -1 on EOF. Otherwise an Unicode character code within the range 0 to
     * 0xFFFF.
     */
     ...
     *
     * <p>On Unix this method switches the console back to echo mode. read() leaves the console in non-echo mode.
     */
