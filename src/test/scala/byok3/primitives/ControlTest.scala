...
class ControlTest extends PrimitivesTestBase {

  test("should throw an error") {
    val ops = sequence(dataStack(push(3)), dataStack(push(-3)), returnStack(push(9)), Control.THROW)
    val ex = ops.runS(emptyContext).failed.get
    ex shouldEqual Error(-3)
  }

  test("should conditionally throw an error when NOS non-zero") {
    val ops = sequence(dataStack(push(3)), dataStack(push(-6)), Control.`?ERROR`)
    val ex = ops.runS(emptyContext).failed.get
    ex shouldEqual Error(-6)
  }

  test("should not throw an error when NOS zero") {
    val ops = sequence(dataStack(push(0)), dataStack(push(-6)), Control.`?ERROR`, dataStack(push(19)))
